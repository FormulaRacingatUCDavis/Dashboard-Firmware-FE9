...
void can_receive(uint8_t *msg, int ID)
{
    ...
    switch (ID) 
    {
        case 0x0566: // Curtis Status
            CAPACITOR_VOLT = msg[CAN_0_RX_DATA_BYTE1(ID)];
            ABS_MOTOR_RPM = msg[CAN_0_RX_DATA_BYTE5(ID)];
            break;
        ...
        case 0x0666:    // pdoAwk from motor controller
            ACK_RX = msg[CAN_0_RX_DATA_BYTE1(ID)];
            break;
        case 0x0201:    // BSPD (brake position from pedal node)
            ERROR_TOLERANCE = msg[CAN_0_RX_DATA_BYTE1(ID)];
            break;
        case 0x0200:    // throttle
            pedalOK = 0x0;
            THROTTLE_HIGH = data[CAN_0_RX_DATA_BYTE2(ID)];
            THROTTLE_LOW = data[CAN_0_RX_DATA_BYTE3(ID)];
            break;
        case 0x388:    // BMS voltage data 
            VOLT_B4 = data[CAN_0_RX_DATA_BYTE5(ID)];
            VOLT_B3 = data[CAN_0_RX_DATA_BYTE6(ID)];
            VOLT_B2 = data[CAN_0_RX_DATA_BYTE7(ID)];
            VOLT_B1 = data[CAN_0_RX_DATA_BYTE8(ID)];
            voltage = (VOLT_B4 << 24) | (VOLT_B3 << 16) | (VOLT_B2 << 8) | VOLT_B1; 
            break;
        case 0x0488:   // BMS Temp data
            ERROR_IDX = data[CAN_0_RX_DATA_BYTE6(ID)];
            ERROR_NODE = data[CAN_0_RX_DATA_BYTE7(ID)];
            PACK_TEMP = data[CAN_0_RX_DATA_BYTE8(ID)];
            tempAttenuate();
            break;
        case 0x521: // current data from IVT
            current_bytes[0] = data[CAN_0_RX_DATA_BYTE3(ID)];
            current_bytes[1] = data[CAN_0_RX_DATA_BYTE4(ID)];
            current_bytes[2] = data[CAN_0_RX_DATA_BYTE5(ID)];
            current_bytes[3] = data[CAN_0_RX_DATA_BYTE6(ID)];
            CURRENT = (current_bytes[0] << 24) + (current_bytes[1] << 16) + (current_bytes[2] << 8) + current_bytes[3]; // check CAN to be sure
            break;
        case 0x188:
            bms_status = (data[CAN_0_RX_DATA_BYTE3(ID)] << 8) | data[CAN_0_RX_DATA_BYTE4(ID)];  
            break;
    }
...
void can_test_send()
{
    ...
    CAN_0_SendMsg(&HeartB);
}

void can_send(uint8_t data[8], uint32_t ID)
{
    uint8_t i;
	CAN_0_TX_MSG message;
	CAN_0_DATA_BYTES_MSG payload;
	message.id = ID; 
	message.rtr = 0;
	message.ide = 0;
	message.dlc = 0x08;
